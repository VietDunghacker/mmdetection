...
class KeypointHead(AnchorFreeHead):
	...
	def __init__(self,
		...
		super(KeypointHead, self).__init__(num_classes, in_channels, init_cfg = init_cfg, **kwargs)
	...
	def _init_layers(self) -> None:
		...
		self.shared_layers = self._init_layer_list(self.in_channels, self.shared_stacked_convs)
		...
		for head_type in self.head_types:
			...
			# head
			keypoint_cls_head = self._init_layer_list(self.feat_channels, self.logits_convs)
	...
	def _get_targets_single(
		...
		offset_target, score_target, pos_mask = assigner.assign(points, num_points, gt_points, gt_bboxes, gt_labels, self.num_classes)
