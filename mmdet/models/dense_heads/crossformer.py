...
class Attention(nn.Module):
	...
	def forward(self, x, H, W, mask=None):
		...
		if self.position_bias:
			...
			biases = biases.flatten(1).transpose(0, 1).contiguous().float().to(attn.device)
			...
			relative_position_index = relative_coords.sum(-1).to(attn.device)  # Gh*Gw, Gh*Gw

			pos = self.pos(biases.type_as(x)) # 2Gh-1 * 2Gw-1, heads
