	...
	def forward(self, input):
		...
		for i, spec in enumerate(self._block_specs):
			if self.with_cp and feat.requires_grad:
				target_feat = cp.checkpoint(self.merge_ops[i], [block_feats[feat_idx] for feat_idx in spec.input_offsets])
			else:
			...
			if spec.is_output:
				for j, (j_feat, j_connections) in enumerate(zip(block_feats, num_outgoing_connections)):
					if j_connections == 0 and j_feat.shape == target_feat.shape:
						target_feat += j_feat
						num_outgoing_connections[j] += 1
			target_feat = F.relu(target_feat, inplace=True)
			if self.with_cp and feat.requires_grad:
				target_feat = cp.checkpoint(self.scale_permuted_blocks[i], target_feat)
			else:
