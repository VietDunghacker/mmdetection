...
def eiou_loss(pred, target, eps=1e-7):
	...
	aspect_loss = (w2 - w1) ** 2 / cw ** 2 + (h2 - h1) ** 2 / ch ** 2

	# EIoU
	eious = ious - (distance_loss + aspect_loss)
	loss = 1 - eious
	return loss
...
def cdiou_loss(pred, target, eps=1e-7):
	...
	ae = ((b2_x1 - b1_x1) ** 2 + (b2_y1 - b1_y1) ** 2).sqrt()
	bf = ((b2_x2 - b1_x2) ** 2 + (b2_y1 - b1_y1) ** 2).sqrt()
	cg = ((b2_x2 - b1_x2) ** 2 + (b2_y2 - b1_y2) ** 2).sqrt()
...
@LOSSES.register_module()
class CDIoULoss(nn.Module):

	def __init__(self, eps=1e-6, reduction='mean', loss_weight=1.0):
		super(CDIoULoss, self).__init__()
		self.eps = eps
		self.reduction = reduction
		self.loss_weight = loss_weight
	...
	def forward(self,
		...
		if weight is not None and weight.dim() > 1:
			...
			assert weight.shape == pred.shape
			weight = weight.mean(-1)
		loss = self.loss_weight * cdiou_loss(
			pred,
			target,
			weight,
			eps=self.eps,
