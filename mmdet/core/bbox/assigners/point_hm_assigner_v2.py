...
class PointHMAssignerV2(BaseAssigner):
	...
	def assign(self, points, gt_bboxes, gt_labels=None):
		...
		if num_points == 0 or num_gts == 0:
			...
			assigned_gt_hm_ct = points.new_zeros((num_points,), dtype=dtype)
			...
			assigned_gt_offset_ct = points.new_zeros((num_points, 2), dtype=torch.float32)
			...
			pos_inds_ct = torch.nonzero(assigned_gt_hm_ct == 1, as_tuple=False).squeeze(-1).unique()
			...
			neg_inds_ct = torch.nonzero(assigned_gt_hm_ct < 1, as_tuple=False).squeeze(-1).unique()
			...
			return assigned_gt_hm_tl, assigned_gt_offset_tl, pos_inds_tl, neg_inds_tl, \
				   ...
				   assigned_gt_hm_ct, assigned_gt_offset_ct, pos_inds_ct, neg_inds_ct
		...
		gt_bboxes_xct = (gt_bboxes_xtl + gt_bboxes_xbr)/2.0
		gt_bboxes_yct = (gt_bboxes_ytl + gt_bboxes_ybr)/2.0

		gt_bboxes_xytl = torch.cat([gt_bboxes_xtl, gt_bboxes_ytl], -1)
		gt_bboxes_xybr = torch.cat([gt_bboxes_xbr, gt_bboxes_ybr], -1)
		gt_bboxes_xyct = torch.cat([gt_bboxes_xct, gt_bboxes_yct], -1)
		...
		distances_ct = (points_xy[:, None, :] - gt_bboxes_xyct[None, :, :]).norm(dim=2)
		...
		for gt_lvl in lvls:
			...
			lvl_distances_ct = distances_ct[lvl_idx, :]
			...
			_, min_dist_index_ct = lvl_distances_ct.min(dim=0)
			min_dist_points_index_ct = points_index[min_dist_index_ct]
			assigned_gt_offset_ct[min_dist_points_index_ct, :] = \
				(gt_bboxes_xyct - lvl_points[min_dist_index_ct, :]) / downscale_factor
			if self.gaussian_bump:
				...
				out_index_ct = lvl_distances_ct >= radius[None, :]
				lvl_gaussian_ct = torch.exp(-torch.pow(lvl_distances_ct, 2) / (2 * sigma * sigma)[None, :])
				lvl_gaussian_ct[out_index_ct] = -INF
				max_gaussian_ct, _ = lvl_gaussian_ct.max(dim=1)
				assigned_gt_hm_ct[points_index[max_gaussian_ct != -INF]] = max_gaussian_ct[max_gaussian_ct != -INF]
			assigned_gt_hm_tl[min_dist_points_index_tl] = 1
			assigned_gt_hm_br[min_dist_points_index_br] = 1
			assigned_gt_hm_ct[min_dist_points_index_ct] = 1
