	...
	dict(
		...
		policies = [
			[
				...
				dict(type='Resize', img_scale=[(370, 370), (960, 960)], multiscale_mode='range', keep_ratio=True),
			],
			[
				...
				dict(
					...
					transforms=[
						dict(
							type = "OneOf",
							transforms=[
								dict(type = "Crop", x_min = 0, y_min = i, x_max = 960, y_max = 960) for i in range(437, 720, 10)
								],
							p=1.0),
						dict(type='ShiftScaleRotate', shift_limit=0.0625, scale_limit=0.1, rotate_limit=45, interpolation=1, p=0.5, border_mode = 0),					
						],
